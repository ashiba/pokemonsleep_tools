(function () {
  ...
  function renderReserveSection() {
    ...
    // チップ: 残り = 所持 − 確保（最大値） マイナスは警告表示（並びは ingredients.json 順）
    ...
    if (els.reserveWarn) {
      ...
      const negNames = [];
      for (const name of state.ingredients) {
        const remain = (ownedRaw[name] || 0) - (reserve[name] || 0);
        if ((ownedRaw[name] || 0) === 0 && (reserve[name] || 0) === 0) continue;
        if (remain < 0) {
          hasNeg = true;
  ...
  function updateSelectedSummary() {
    ...
    const orderMap = new Map(state.ingredients.map((n, i) => [n, i]));
    const sorted = Object.entries(maxCounts).sort((a, b) => (orderMap.get(a[0]) ?? 999) - (orderMap.get(b[0]) ?? 999));
